...
조건부 후보의 2차 확장 목표가 일정 수준 이상인 메시지만 받으려면
...
거래량이 이전 20봉 평균과 직전 봉 대비 유지되며, 종가 위치·윗꼬리·추격 여부·유동성과
손익비 같은 핵심 안전조건을 통과한 경우에만 후보를 생성합니다.

저항 여유·호가 지지·스프레드는 두 단계로 판정합니다. 저항 여유 2.5% 미만,
호가비율 0.4 미만이 표본 과반, 스프레드 1.2% 초과처럼 극단적인 경우는 탈락합니다.
그보다 양호하지만 선호 기준인 저항 5%, 호가비율 0.8, 스프레드 0.5%에 미달하면
각각 6점·4점·4점을 감점하고 위험 메모와 5% 이내 비중을 표시합니다.
...
뜻입니다. 일반 후보는 가까운 구조적 저항을 고려한 2~3% 1차 목표와, 1차 저항 돌파를
전제로 한 5% 2차 확장 목표를 사용합니다. 조건점수 95점 이상, 30분 횡보 재돌파,
거래량·추세·BTC·호가·저항 여유까지 모두 강한 경우에만 7%·10% 강한 추세 확장형을
표시합니다.
...
축소로 반영합니다. 거래량 소멸, 긴 윗꼬리, 추격 구간, 극단적으로 가까운 저항과
불리한 손익비 같은 직접적인 진입 위험은 계속 하드 필터로 유지합니다.
