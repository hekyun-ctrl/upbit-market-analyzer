...
분석에 전달되며 동일한 30초 재검증·과열·호가·추격 방지 필터를 거칩니다.
...
점수 90점 이상인 고점 돌파·횡보 재돌파 신호 중 1분·5분 이동평균 상승 배열,
완료 1분봉 거래량 1.5배 이상 지속, RSI 비과열 조건까지 충족한 경우에만
`강한 추세 확장형`으로 분류하여 1차 약 7%, 2차 약 10% 목표를 표시합니다.
그 밖의 후보는 도달 가능성을 고려해 기존 위험비 기준 목표를 유지합니다. 높은 목표가는
수익을 보장하지 않으며, 실제 저항과 거래량 약화가 확인되면 별도 재검토가 필요합니다.
