...
5·15·60분 모멘텀을 같은 시점에 비교합니다. 기본적으로 상대강도 상위 15%이면서
5분 상승률이 0.8% 이상인 종목만 심층 후보를 통과할 수 있습니다. 시장 순위가 아직
...
보호와 기본 8%의 2차 추세 목표를 함께 안내합니다.

정규 후보 생성 기준은 90점입니다. 다만 하루 후보 가용성을 위해 84~89점 중 과열·BTC
약세가 아니고, 호가 지지·스프레드·저항·손익비 등 직접 안전선을 모두 통과한 후보는
`보완형`으로 분리합니다. 보완형은 5% 이내 비중으로 표시되며 정규 후보와 혼동되지
않도록 Telegram 제목과 위험 감점에 명시됩니다.

메시지 가용성을 위해 비과열 초기 신호에는 적응형 품질 판정을 적용합니다. 당일 상승률
10% 미만이고 1분 RSI 70·5분 RSI 68 미만인 신호는 거래량·종가 위치·윗꼬리·1분
20이평 중 경미한 미달을 최대 2개까지 자동 탈락 대신 조건점수 감점으로 처리합니다.
절대 하한 미달, 5분 20이평 이탈, 극단적 과열·스프레드, 돌파 실패는 계속 즉시
탈락합니다. 30분 횡보 재돌파는 BTC가 약하지 않고 완료봉 품질이 강한 경우에 한해
구조적 저항 여유 1.5~2.5% 구간도 감점형 후보로 검토합니다. 이 기준은 메시지 최소
개수를 강제로 채우지 않으면서 정상적인 상승 초입의 과도한 누락을 줄이기 위한 것입니다.
...
중간 과열 구간에서도 거래량·이동평균·돌파·저항 여유·손익비를 모두 통과하고 완료봉의
종가 위치와 윗꼬리만 경미하게 기준에 미달한 경우에는 최대 2개까지 감점형 후보로
검토합니다. 급락 이력, 거래량 절대 부족, 돌파 실패와 가까운 저항은 완화하지 않습니다.
...
한국시간 하루 기준 후보 메시지는 5~10개를 목표로 관리합니다. 90점 이상 정규 후보는
하루 최대 10개까지 전송하고, 보완형 후보는 하루 5개 목표를 채우기 전까지만 최소
90분 간격으로 허용합니다. 하루 10개에 도달하면 모든 후보 전송을 다음 날까지
...
있습니다. 이와 별도로 모든 전송 후보를 최대 6시간 추적해 1차·2차 목표, 1차 뒤
진입가 보호, 최대 유리·불리 변동을 `six_hour_trend_performance`에 기록합니다.
